...
file='https://www.gutenberg.org/files/58785/58785-0.txt'
echo `curl ${file}`
...
sorted_words2=`curl -s $1|tr [A-Z] [a-z]|grep -oE "\w+"|sort|uniq -c|sort -k 2 -r`
# c1=`sort "$sorted_words" | -k 1`
# c2=`sort "$sorted_words" | -k 2 -r`
...
mini=`echo "$sorted_words"| head -n 1`
maxi=`echo "$sorted_words"| tail -n 1`

echo "Min frequency and word   $mini"
echo "Max frequency and word  $maxi"
...
nums=`echo "$sorted_words"|cut -f 1 `
med=`echo sort -n "$nums" | awk ' { a[i++]=$1; } END { print a[int(i/2)]; }'`
...
mn=`echo $((count/total_freq))|bc`
echo "Mean frequency using floating point arithemetics =  $mn"
